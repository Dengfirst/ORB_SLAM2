    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    cv::Mat K = cv::Mat::eye(3,3,CV_32F);       //CV_32F 代表32 位浮点数
    K.at<float>(0,0) = fx;
    K.at<float>(1,1) = fy;
    K.at<float>(0,2) = cx;
    K.at<float>(1,2) = cy;
    ...
    float fps = fSettings["Camera.fps"];     //FPS 摄像头帧率
    if(fps==0)
        fps=30;
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        KeyFrame* pKFini = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB);  //把关键点分配到网格中，设置双目相机光心的世界坐标、相机坐标系原点的世界坐标
        ...
        for(int i=0; i<mCurrentFrame.N;i++)
        {
            ...
            if(z>0)
            {
                ...
                cv::Mat x3D = mCurrentFrame.UnprojectStereo(i);      // 通过运动方程更新3D点
                // 步骤4.2：将3D点构造为MapPoint
                MapPoint* pNewMP = new MapPoint(x3D,pKFini,mpMap);   // pNew 相当于特征点的世界坐标
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    ORBmatcher matcher(0.7,true);          // 参数0.7 是最高分与第二高分的比
    vector<MapPoint*> vpMapPointMatches;   // vpMapPointMatches代表当前帧中MapPoints对应的匹配

    // 步骤2：通过特征点的BoW加快当前帧与参考帧之间的特征点匹配
    // 特征点的匹配关系由MapPoints进行维护
    int nmatches = matcher.SearchByBoW(mpReferenceKF,mCurrentFrame,vpMapPointMatches);  // nmatches为匹配成功的数量
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(vDepthIdx[j].first>mThDepth && nPoints>100)    //vDepthIdx在步骤2.2进行排序了，如果距离比较近的第i点满足条件后面i+1个点都满足
            break;
    }
}
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // 剔除异常值点后，余的匹配依然nmatchesMap>=10 对，则跟踪成功，否则跟踪失败
    if(mbOnlyTracking)
    {
        mbVO = nmatchesMap<10;
        return nmatches>20;
...
bool Tracking::TrackLocalMap()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!mCurrentFrame.mvbOutlier[i])
            {
                mCurrentFrame.mvpMapPoints[i]->IncreaseFound();    //当前帧能观察到的地图点+1
                if(!mbOnlyTracking)
                {
                    // 该MapPoint被其它关键帧观测到过
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                // 步骤6：如果内点(即nGood)较少，则通过投影的方式对之前未匹配的点进行匹配，再进行优化求解
                if(nGood<50)
                {
